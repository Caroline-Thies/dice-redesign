...
export default function AppView(props) {
    ...
    useEffect(() => setAvailableDice(kgLevel + concentration + timelessKnowledge - investedDice), [concentration, timelessKnowledge, investedDice])
    ...
    return <div className="flexcol">
        ...
        <div>
           <h4>Konzentration</h4>
           <NumberPicker name="Konzentration" max={maxConcentration} selected={concentration} onChange={setConcentration} startsWithZero={true}/>
        </div>
        <div>
            <h4>Zeitloses Wissen</h4>
            <NumberPicker name="Zeitloses Wissen" max={5} selected={timelessKnowledge} onChange={setTimelessKnowledge} startsWithZero={true} />
        </div>
        <div>
            <h4>Investierte Würfel</h4>
            <NumberPicker name="Investierte Würfel" max={kgLevel + concentration + timelessKnowledge} selected={investedDice} startsWithZero={true} onChange={setInvestedDice} />
        </div>
        <div className="textContainer">
            ...
            liegt dann bei ca. <span className="greenText large">{Math.round(successRate * 100) / 100}%</span>.
            </p>
        </div>
        {renderDiceResult()}
