...
export default function AppSelectView(props) {
  ...
  const getApps = () => {
    ...
    if (!filterActive) {
      ...
      selectedKraftgruppen.map(
        (kraftgruppe) => (kgApps = kgApps.concat(apps[kraftgruppe + "kinese"]))
      );
    }
        ...
        <div className="appList">
          {apps.map((app) => (
            <AppCard
              ...
              key={app.name}
              onClick={() =>
                navigate("/" + params.characterName + "/" + app.name, {
                  state: {
                    app: app,
                    kraftgruppe: kraftgruppen[app.kraftgruppe],
                    wisdom: wisdom,
                  },
                })
              }
    ...
    <div className="flexcol">
      <div className="flexrow sticky">
        ...
        <img
          ...
          alt="click here to toggle filter"
        />
      </div>
      {filterOptions()}
      {showFilterResults()}
