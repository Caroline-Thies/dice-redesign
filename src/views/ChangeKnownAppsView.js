...
export default function ChangeKnownAppsView() {
  ...
  const [appsByKraftgruppe, setAppsByKraftgruppe] = useState({
    Animakinese: [],
    Audiokinese: [],
    Biokinese: [],
    Chronokinese: [],
  ...
  const selectKraftgruppe = async (kgList) => {
    ...
    if (appsByKraftgruppe[kraftgruppe].length === 0) {
      const appsByKraftgruppeCopy = JSON.parse(
        JSON.stringify(appsByKraftgruppe)
      );
      appsByKraftgruppeCopy[kraftgruppe] =
        await BackendAdapter.getAllAppNamesByKraftgruppe(kraftgruppe);
  ...
  return (
    <div className="flexcol">
      ...
      <KraftgruppenPicker
        radio={true}
        updateSelectedKraftgruppen={selectKraftgruppe}
      />
      <h4>Apps</h4>
      <AppPicker
        apps={appsByKraftgruppe[selectedKraftgruppe]}
        knownApps={knownApps}
