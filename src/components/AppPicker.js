...
export default function AppPicker(props) {
  ...
  useEffect(() => {
    if (props.apps.length > 0 && props.knownApps.length > 0)
      setCheckedState(props.apps.map((app) => props.knownApps.includes(app)));
  }, [props.apps, props.knownApps]);
      ...
      {apps.map((kg, index) => {
        ...
        return (
          <div key={key}>
            ...
            />
            <label className="radio__label appBox noselect" htmlFor={key}>
              {value}
            </label>
          </div>
        );
